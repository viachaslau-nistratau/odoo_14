...
class Member(models.Model):
    ...
    card_number = fields.Char(
        string='Номер абонемента пользователя',
    ...
    count_book = fields.Integer(
        string='Количество книг на руках'
    )

    # Блок персональной информации
    personal_info = fields.Boolean(
        string='Персональная информация'
    )
    home_address_member = fields.Char(
        string='Домашний адрес',
        size=100,
    )
    mobil_phone = fields.Char(
        string='Номер мобильного телефона',
        size=15,
    )
    home_phone = fields.Char(
        string='Номер домашнего телефона',
        size=15,
    )

    # Блок информации о месте работы
    info_about_place_job = fields.Boolean(
        string='Информация о месте работы',
    )
    job_member = fields.Char(
        string='Место работы',
    )
    ...
    name_book = fields.Char(
        string='Книги', translate=True,
        required=True,
    )
    member_book_ids = fields.One2many(
        'library.book',
        'member_book_id',
        string='',
    )
    ...
    date_take_book = fields.Date(
        string='Дата получения книги',
    )
    date_return = fields.Date(
        string='Дата возврата книги',
    )
    # compute='date_return_book',

    alarm_date = fields.Char(
        string='Примечание',

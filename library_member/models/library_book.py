...
class Book(models.Model):
    ...
    # определяем его снова и в какой - то момент можем использовать super()
    # для вызова существующей реализации метода. В нашем методе мы проверяем,
    # является ли это 10-значным ISBN, и в этом случае выполняем недостающую
    # логику проверки. В противном случае мы вернемся к оригинальной логике
    # проверки ISBN, которая может обрабатывать 13-значный регистр.

...
class Books(http.Controller):
    ...
    # вошел в систему. Рекомендуется явно ссылаться на маршрут режим авторизации,
    # поэтому мы добавили аргумент auth = 'user'. Мы можем позволить анонимный доступ,
    # добавив вместо этого параметр auth = 'public' в @http.route.
    @http.route('/library/books', auth='user')
    def list(self, **kwargs):
        # Внутри метода контроллера мы получаем доступ к среде с помощью http.request.env.
        # Мы используем его, чтобы получить набор записей со всеми активными книгами в каталоге.
